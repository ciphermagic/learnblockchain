 ...
 * ## 🔍 模拟工作原理
 *
 * 模拟交易的核心思路是：执行交易 → 分析结果 → 回滚状态
 *
 * ```
 * 1. 创建快照 (evm_snapshot)     # 保存当前状态
 * 2. 执行交易 (eth_sendTransaction)  # 在模拟环境中执行
 * 3. 获取交易回执 (eth_getTransactionReceipt)  # 获取执行结果
 * 4. 分析日志和事件
 *    - 解析 Transfer 事件
 *    - 追踪内部调用（可选）
 * 5. 回滚到快照 (evm_revert)    # 恢复原始状态
 * 6. 返回结果                   # 报告给用户
 * ```
 *
 * ## 使用的 RPC 方法
 ...
 * | 方法 | 说明 |
 * |------|------|
 * | `evm_snapshot` | 创建状态快照，保存当前区块链状态 |
 * | `evm_revert` | 恢复到指定快照，撤销所有状态更改 |
 * | `eth_sendTransaction` | 发送交易（通过钱包签名） |
 * | `eth_getTransactionReceipt` | 获取交易回执，包含执行结果 |
 * | `eth_estimateGas` | 估算交易需要的 Gas 数量 |
 * | `trace_transaction` | Parity/Erigon 风格的交易追踪 |
 * | `debug_traceTransaction` | Geth 风格的调试追踪 |
 * | `debug_traceCall` | Geth 风格的模拟调用（不执行） |
 *
 * ## 模拟方法对比
 *
 * ### 方法 1: 基础模拟
 ...
 * - 缺点：只能追踪顶层 ETH 转账
 *
 * ### 方法 2: trace_transaction
 * - Parity/Erigon 风格的追踪
 * - 优点：能追踪所有 ETH 转账
 * - 缺点：需要先执行交易
 *
 * ### 方法 3: debug_traceTransaction
 * - Geth 风格的调试追踪
 * - 优点：详细、可配置
 * - 缺点：需要先执行交易
 *
 * ### 方法 4: debug_traceCall
 * - 不执行交易的模拟调用
 * - 优点：不需要快照，可直接模拟
 * - 缺点：某些场景可能不准确
 *
  ...
  private extractAllTransfersFromTrace(traces: any): ETHTransfer[] {
    ...
    if (Array.isArray(traces)) {
      for (const trace of traces) {
        ...
        if (trace.type === 'call' && trace.action?.value && BigInt(trace.action.value) > 0n) {
          transfers.push({
            type: 'ETH',
