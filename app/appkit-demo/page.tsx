...
import { useAccount, useDisconnect, useChainId, useChains, useReadContract, useWriteContract, useBalance } from 'wagmi';
        ...
        {!isConnected ? (
          ...
          <div className='space-y-4'>
            ...
            <div className='text-center'>
              ...
              <p className='font-mono'>
                {balance ? `${formatUnits(balance.value, balance.decimals)} ${balance.symbol}` : '0 ETH'}
              </p>
            </div>
            <div className='text-center'>
              ...
              <button
                ...
                className={`mt-2 w-full py-2 px-4 rounded transition-colors ${
                  isPending ? 'bg-gray-400 cursor-not-allowed' : 'bg-green-500 hover:bg-green-600 text-white'
                }`}
              >
                {isPending ? '处理中...' : '增加计数'}
              </button>
              {isPending && <p className='mt-2 text-gray-600'>交易正在处理中...</p>}
              {hash && <p className='mt-2 text-blue-500'>交易哈希: {hash}</p>}
              {isError && <p className='mt-2 text-red-500'>错误: {error?.message}</p>}
            </div>
            <button
              onClick={() => disconnect()}
