...
contract OracleSimpleTest is Test {
    ...
    // 测试参数
    string constant public SYMBOL = "MEME";
    uint256 constant public TOTAL_SUPPLY = 1000000 * 10 ** 18; // 1,000,000 tokens
    uint256 constant public PER_MINT = 1000 * 10 ** 18;       // 1,000 tokens per mint
    uint256 constant public PRICE = 0.0001 ether;           // 0.0001 ETH per token
    ...
    function setUp() public {
        ...
        // 部署 Meme_Factory 合约
        factory = new MemeFactoryV2(projectOwner, address(router));
    ...
    function testMultipleUpdates() public {
        ...
        for (uint i = 1; i <= 3; i++) {
            ...
            for (uint j = 1; j <= 5; j++) {
                // 每次交易推进4小时，但以初始时间为基础
                vm.warp(initialTimestamp + (i - 1) * 24 hours + j * 4 hours);
            ...
            // 确保距离上次更新至少经过了一个完整的PERIOD（24小时）
            // 在第一次循环中，时间将从 initialTimestamp + 24小时+1秒开始
            // 在后续循环中，每次增加24小时
            vm.warp(initialTimestamp + i * 24 hours + 1 seconds);
    ...
    function testPriceVolatility() public {
        ...
        vm.warp(initialTimestamp + 5 hours);  // +4小时，总共6小时
        simulateTrading(1000 * 10 ** 18, 1 ether);

        // 价格暴跌：1 Meme = 0.00001 ETH (1/100 初始价格)
        vm.warp(initialTimestamp + 12 hours); // +7小时，总共13小时
        simulateTrading(10000 * 10 ** 18, 0.1 ether);

        // 价格回归：1 Meme = 0.0001 ETH
        // 这里保持交易，但确保在调用update前经过完整周期
        vm.warp(initialTimestamp + 13 hours); // +1小时，总共14小时
        simulateTrading(1000 * 10 ** 18, 0.1 ether);

        // 前进到 PERIOD 之后 (24小时后)
        vm.warp(initialTimestamp + 24 hours + 1 seconds);
