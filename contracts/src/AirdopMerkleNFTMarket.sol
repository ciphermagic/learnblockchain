...
contract AirdopMerkleNFTMarket is IERC1363Receiver {
  ...
  function buyNFT(uint256 _listingId) external {
    ...
    require(
      ...
      'AirdopMerkleNFTMarket: token transfer failed'
    );
   ...
   *
   *      NFT成交机制说明：
   *      - 本市场采用“非托管上架”模式，NFT在上架时不会转入市场合约
   *      - list() 只是记录上架信息，NFT仍然保存在卖家地址中
   *      - 真正成交时，市场合约才会调用 transferFrom(listing.seller, buyer, tokenId)
   *      - 因此卖家必须在成交前授权市场合约转移该NFT，否则NFT转移会失败
   *
   *      授权要求：
   *      - 卖家可通过 approve(marketAddress, tokenId) 授权单个NFT
   *      - 或通过 setApprovalForAll(marketAddress, true) 授权整个NFT系列
   *      - 如果卖家尚未授权市场合约，即使listing处于active状态，claimNFT仍会在NFT转移时失败并回滚
   *
   *      上架与授权的关系：
   *      - 本合约允许“先上架，后授权市场合约”的流程
   *      - 即：NFT所有者或其授权者可先调用list()完成上架
   *      - 之后由NFT所有者再授权市场合约
   *      - 最终买家调用claimNFT()时完成实际成交
   *      - 因此，listing active 仅表示该NFT处于可尝试购买状态，不保证一定能立即成交
   *
   *      状态更新顺序：
   *      - 在转移代币和NFT之前，先标记hasUsedWhitelist和listing.isActive
   *      - 这样可以防止重复购买或重复使用白名单优惠
   *      - 若后续任一步转账失败，整笔交易会revert，状态也会一并回滚
   *
   *      Multicall典型用法：
   *      - 用户可先在链下签名permit授权消息
   *      - 然后通过multicall将“授权 + 购买”合并为一笔交易：
   *      multicall([
